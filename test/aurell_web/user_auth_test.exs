defmodule AurellWeb.UserAuthTest do
  use AurellWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Aurell.Accounts
  alias Aurell.Accounts.Scope
  alias AurellWeb.UserAuth

  import Aurell.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, AurellWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      AurellWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        AurellWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
